...
class PermissionBasedFilterBackend(FilterBackend):
    ...
    def filter_queryset(self, request, queryset, view):
        ...
        if request.user.is_authenticated() and request.user.is_superuser:
            ...
            qs = qs.filter(Q(project__owner=request.user) | Q(project_id__in=projects_list) | Q(project__public_permissions__contains=[self.permission]))
        else:
...
class CanViewProjectObjFilterBackend(FilterBackend):
    def filter_queryset(self, request, queryset, view):
        ...
        if request.user.is_authenticated() and request.user.is_superuser:
            ...
            qs = qs.filter(Q(owner=request.user) | Q(id__in=projects_list) | Q(public_permissions__contains=["view_project"]))
        else:
